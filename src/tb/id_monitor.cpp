...
void id_monitor::monitor_thread() {
    ...
    while (1) {
        ...
        cout << "RF_access " << rf_access /*<< " write_en " << rf_wr_nrd*/
        << " Instr_decoding_en " << decode_en << endl;
        cout << "PC " << unsigned(pc_in) << " Instruction "
                << OPCODE_STRING.at(OPCODE) << " " << std::bitset<32>(instr)
                << " Row " << row_addr << " Column " << col_addr << endl;
        cout << "PC_rst " << pc_rst << " Count_en " << count_en << " Jump_en "
                << jump_en << " Jump_num " << unsigned(jump_num) << endl;
        cout << "CRF_wr_en " << crf_wr_en << " CRF_wr_addr " << crf_wr_addr
                << endl;
        cout << "SRF_rd_addr " << srf_rd_addr << " SRF_rd_a_nm " << srf_rd_a_nm
                << " SRF_wr_en " << srf_wr_en << " SRF_wr_addr " << srf_wr_addr
                << " SRF_wr_a_nm " << srf_wr_a_nm << " SRF_wr_from "
                << MUX_STORAGE_STRING.at(srf_wr_from) << endl;
        cout << "GRFA_rd_addr1 " << grfa_rd_addr1 << " GRFA_rd_addr2 "
                << grfa_rd_addr2 << " GRFA_wr_en " << grfa_wr_en
                << " GRFA_relu_en " << grfa_relu_en << " GRFA_wr_addr "
                << grfa_wr_addr << " GRFA_wr_from "
                << MUX_STORAGE_STRING.at(grfa_wr_from) << endl;
        cout << "GRFB_rd_addr1 " << grfb_rd_addr1 << " GRFB_rd_addr2 "
                << grfb_rd_addr2 << " GRFB_wr_en " << grfb_wr_en
                << " GRFB_relu_en " << grfb_relu_en << " GRFB_wr_addr "
                << grfb_wr_addr << " GRFB_wr_from "
                << MUX_STORAGE_STRING.at(grfb_wr_from) << endl;
        cout << "FPU_mult_en " << fpu_mult_en << " FPU_add_en " << fpu_add_en
                << " FPU_mult_in1_sel " << MUL1_SEL_STRING.at(fpu_mult_in1_sel)
                << " FPU_mult_in2_sel " << MUL2_SEL_STRING.at(fpu_mult_in2_sel)
                << " FPU_add_in1_sel " << ADD_SEL_STRING.at(fpu_add_in1_sel)
                << " FPU_add_in2_sel " << ADD_SEL_STRING.at(fpu_add_in2_sel)
                << " FPU_out_sel " << fpu_out_sel << endl << endl;
