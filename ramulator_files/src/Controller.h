...
public:
    ...
    bool wm_blocked = false;  // HBM_AB blocks write mode if requests were upgraded to activation queue
    ...
    bool enqueue_AB(Request& req){
        ...
        // AllBanks memories ensure command order, so not needed to control RD after WR hazards
    ...
    void tick_AB(){
        ...
        /*** 3. Should we schedule writes? ***/
        // AllBanks memories maintain command order
        if (!wm_blocked) {
        ...
        if(is_valid_req)    wm_blocked = false;
        ...
        if (cmd != channel->spec->translate[int(req->type)]) {
            if(channel->spec->is_opening(cmd)) {
                ...
                actq.q.push_back(*req);
                queue->q.erase(req);
                // AllBanks: added for maintaining order
                if (req->type == Request::Type::READ) {
                    write_mode = false;
                    wm_blocked = true;
                }
        ...
        // AllBanks: added for maintaining cmd order
        // If both queues have elements, order by arrival time
        if (readq.size() && writeq.size()) {
            if (scheduler->get_head(writeq.q)->arrive <= scheduler->get_head(readq.q)->arrive)
                write_mode = true;
            else
                write_mode = false;
        // Else, change mode to the one of the populated queue
        } else if (readq.size()) {
            write_mode = false;
        } else if (writeq.size()) {
            write_mode = true;
