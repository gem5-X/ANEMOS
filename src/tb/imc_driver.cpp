...
void imc_driver::driver_thread() {
    ...
    for (j = 0; j < 4; j++) {
        ...
        for (k = 0; k < 4; k++) {
            half_aux = new half(
                    pow(-1.0, k + 1) * (31.0 * j + 23.0 * k + -14.0));
            DQ_tosend |= ((uint64_t) half_aux->bin_word()) << k * 16;
        }
        DQ->write(DQ_tosend);
        wait(clk_period, RESOLUTION);
    ...
    for (i = 0; i < 8; i++) {
        if (i > 1) {
            ...
            for (j = 0; j < 4; j++) {
                ...
                for (k = 0; k < 4; k++) {
                    half_aux = new half(
                            0.016 * i + 0.004 * j + 0.001 * k + 0.0001);
                    DQ_tosend |= ((uint64_t) half_aux->bin_word()) << k * 16;
                }
                DQ->write(DQ_tosend);
                wait(clk_period, RESOLUTION);
    ...
    AB_mode->write(false);
    pim_mode->write(true);
    row_aux.range(ROW_BITS - 1, ROW_BITS - 1) = false;	// Executing the PIM
    row_addr->write(row_aux);
    wait(
            (8 * (2 + std::max(1, ADD_STAGES - 1)) + 18 + MULT_STAGES
                    + ADD_STAGES) * clk_period, RESOLUTION);
    WR->write(false);
    ...
    for (j = 0; j < 4; j++) {
        ...
        for (k = 0; k < 4; k++) {
            half_aux = new half(
                    pow(-1.0, k + 1) * (0.002 * j + 0.000021 * k + -0.00003));
            DQ_tosend |= ((uint64_t) half_aux->bin_word()) << k * 16;
        }
        DQ->write(DQ_tosend);
        wait(clk_period, RESOLUTION);
    ...
    for (i = OPC_GRF_A; i <= OPC_SRF_A; i++) {
        ...
        for (k = 0; k < 16; k++) {
            ...
            bank_tosend |= ((sc_biguint<GRF_WIDTH> ) half_aux->bin_word())
                    << k * 16;
        }
        ...
        for (k = 0; k < 16; k++) {
            half_aux = new half(
                    pow(-1.0, k + i) * (0.0000013 * k + 0.0000005 * i));
            bank_tosend |= ((sc_biguint<GRF_WIDTH> ) half_aux->bin_word())
                    << k * 16;
        }
