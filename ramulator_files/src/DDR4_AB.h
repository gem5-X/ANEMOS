...
public:
    ...
    enum class Org;
    enum class Speed;
    DDR4_AB(Org org, Speed speed);
    DDR4_AB(const string& org_str, const string& speed_str);
    ...
    /* Prereq */
    function<Command(DRAM<DDR4_AB>*, Command cmd, int)> prereq[int(Level::MAX)][int(Command::MAX)];

    // SAUGATA: added function object container for row hit status
    /* Row hit */
    function<bool(DRAM<DDR4_AB>*, Command cmd, int)> rowhit[int(Level::MAX)][int(Command::MAX)];
    function<bool(DRAM<DDR4_AB>*, Command cmd, int)> rowopen[int(Level::MAX)][int(Command::MAX)];
    ...
    /* Lambda */
    function<void(DRAM<DDR4_AB>*, int)> lambda[int(Level::MAX)][int(Command::MAX)];
    ...
    struct SpeedEntry {
        ...
        {3200, 400,  2.500, 1,                      1,     3,    1,    6,  6,   6,  4,   14,  20, 3,   1,    3,    6,  2,    3,    10,  0,   0,    2,  3,  0,     2,     0,  0}
