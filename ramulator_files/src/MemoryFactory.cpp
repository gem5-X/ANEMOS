...
template <>
void MemoryFactory<LPDDR4_AB>::validate(int channels, int ranks, const Config& configs) {
    assert(channels >= 2 && "LPDDR4_AB requires 2, 4, 8 ... channels");
}
...
template <>
void MemoryFactory<HBM_AB>::validate(int channels, int ranks, const Config& configs) {
    assert(channels == 16 && "HBM_AB comes with 16 channels");
}

template <>
void MemoryFactory<HBM2>::validate(int channels, int ranks, const Config& configs) {
    assert(channels == 16 && "HBM2 comes with 16 channels");
}

template <>
void MemoryFactory<HBM2_AB>::validate(int channels, int ranks, const Config& configs) {
    assert(channels == 16 && "HBM2_AB comes with 16 channels");
}
