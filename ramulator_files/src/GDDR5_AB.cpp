...
string GDDR5_AB::standard_name = "GDDR5_AB";
string GDDR5_AB::level_str [int(Level::MAX)] = {"Ch", "Ra", "Bg", "Ba", "Ro", "Co"};


map<string, enum GDDR5_AB::Org> GDDR5_AB::org_map = {
    {"GDDR5_512Mb_x16", GDDR5_AB::Org::GDDR5_512Mb_x16}, {"GDDR5_512Mb_x32", GDDR5_AB::Org::GDDR5_512Mb_x32},
    {"GDDR5_1Gb_x16", GDDR5_AB::Org::GDDR5_1Gb_x16}, {"GDDR5_1Gb_x32", GDDR5_AB::Org::GDDR5_1Gb_x32},
    {"GDDR5_2Gb_x16", GDDR5_AB::Org::GDDR5_2Gb_x16}, {"GDDR5_2Gb_x32", GDDR5_AB::Org::GDDR5_2Gb_x32},
    {"GDDR5_4Gb_x16", GDDR5_AB::Org::GDDR5_4Gb_x16}, {"GDDR5_4Gb_x32", GDDR5_AB::Org::GDDR5_4Gb_x32},
    {"GDDR5_8Gb_x16", GDDR5_AB::Org::GDDR5_8Gb_x16}, {"GDDR5_8Gb_x32", GDDR5_AB::Org::GDDR5_8Gb_x32},
};

map<string, enum GDDR5_AB::Speed> GDDR5_AB::speed_map = {
    {"GDDR5_4000", GDDR5_AB::Speed::GDDR5_4000}, {"GDDR5_4500", GDDR5_AB::Speed::GDDR5_4500},
    {"GDDR5_5000", GDDR5_AB::Speed::GDDR5_5000}, {"GDDR5_5500", GDDR5_AB::Speed::GDDR5_5500},
    {"GDDR5_6000", GDDR5_AB::Speed::GDDR5_6000}, {"GDDR5_6500", GDDR5_AB::Speed::GDDR5_6500},
    {"GDDR5_7000", GDDR5_AB::Speed::GDDR5_7000}, {"GDDR5_300MHz", GDDR5_AB::Speed::GDDR5_300MHz},
};

GDDR5_AB::GDDR5_AB(Org org, Speed speed) : 
...
GDDR5_AB::GDDR5_AB(const string& org_str, const string& speed_str) :
    GDDR5_AB(org_map[org_str], speed_map[speed_str]) 
{
}

void GDDR5_AB::set_channel_number(int channel) {
  org_entry.count[int(Level::Channel)] = channel;
}

void GDDR5_AB::set_rank_number(int rank) {
  assert((rank == 1) && "GDDR5 rank number is fixed to 1.");
}

void GDDR5_AB::init_speed()
{
    const int REFIL_TABLE[int(Speed::MAX)] = {3900, 4388, 4875, 5363, 5850, 6338, 6825, 585};
    const int REFIS_TABLE[int(Speed::MAX)] = {1900, 2138, 2375, 2613, 2850, 3088, 3325, 285};
    const int RFC_TABLE[5][int(Speed::MAX)] = {
        // using DDR3 values
        {90, 102, 113, 124, 135, 147, 158, 14},
        {110, 124, 138, 152, 165, 179, 193, 17},
        {160, 180, 200, 220, 240, 260, 280, 24},
        {260, 293, 325, 358, 390, 423, 455, 39},
        {350, 394, 438, 482, 525, 569, 613, 53}
    };
...
void GDDR5_AB::init_prereq()   // @NOTE to access rows of rest of banks, we could keep the row id and run through the sibling banks and BGs
{                           // However, if we update correctly, this should not be needed
    // RD
    prereq[int(Level::Rank)][int(Command::RD)] = [] (DRAM<GDDR5_AB>* node, Command cmd, int id) {
    ...
    prereq[int(Level::Bank)][int(Command::RD)] = [] (DRAM<GDDR5_AB>* node, Command cmd, int id) {
        switch (int(node->state)) {
    ...
    // REF
    prereq[int(Level::Rank)][int(Command::REF)] = [] (DRAM<GDDR5_AB>* node, Command cmd, int id) {
    ...
    // PD
    prereq[int(Level::Rank)][int(Command::PDE)] = [] (DRAM<GDDR5_AB>* node, Command cmd, int id) {
        switch (int(node->state)) {
    ...
    // SR
    prereq[int(Level::Rank)][int(Command::SRE)] = [] (DRAM<GDDR5_AB>* node, Command cmd, int id) {
        switch (int(node->state)) {
...
void GDDR5_AB::init_rowhit()  // @NOTE to access rows of rest of banks, we could keep the row id and run through the sibling banks and BGs
{                             // However, if we update correctly, this should not be needed
    // RD
    rowhit[int(Level::Bank)][int(Command::RD)] = [] (DRAM<GDDR5_AB>* node, Command cmd, int id) {
        switch (int(node->state)) {
...
void GDDR5_AB::init_rowopen() // @NOTE to access rows of rest of banks, we could keep the row id and run through the sibling banks and BGs
{                             // However, if we update correctly, this should not be needed
    // RD
    rowopen[int(Level::Bank)][int(Command::RD)] = [] (DRAM<GDDR5_AB>* node, Command cmd, int id) {
        switch (int(node->state)) {
            case int(State::Closed): return false;
...
void GDDR5_AB::init_lambda()
{
    lambda[int(Level::Bank)][int(Command::ACT)] = [] (DRAM<GDDR5_AB>* node, int id) {
    ...
    lambda[int(Level::Bank)][int(Command::PRE)] = [] (DRAM<GDDR5_AB>* node, int id) {
        for (auto bg : node->parent->parent->children)  // Close row at all banks at every BG
            for (auto bank: bg->children) {
                bank->state = State::Closed;
                bank->row_state.clear();}};
    lambda[int(Level::Rank)][int(Command::PREA)] = [] (DRAM<GDDR5_AB>* node, int id) {
        for (auto bg : node->children)
            for (auto bank : bg->children) {
                bank->state = State::Closed;
                bank->row_state.clear();}};
    lambda[int(Level::Rank)][int(Command::REF)] = [] (DRAM<GDDR5_AB>* node, int id) {};
    lambda[int(Level::Bank)][int(Command::RD)] = [] (DRAM<GDDR5_AB>* node, int id) {};
    lambda[int(Level::Bank)][int(Command::WR)] = [] (DRAM<GDDR5_AB>* node, int id) {};
    lambda[int(Level::Bank)][int(Command::RDA)] = [] (DRAM<GDDR5_AB>* node, int id) {
        for (auto bg : node->parent->parent->children)  // Close row at all banks at every BG
            for (auto bank: bg->children) {
                bank->state = State::Closed;
                bank->row_state.clear();}};
    lambda[int(Level::Bank)][int(Command::WRA)] = [] (DRAM<GDDR5_AB>* node, int id) {
        for (auto bg : node->parent->parent->children)  // Close row at all banks at every BG
            for (auto bank: bg->children) {
                bank->state = State::Closed;
                bank->row_state.clear();}};
    lambda[int(Level::Rank)][int(Command::PDE)] = [] (DRAM<GDDR5_AB>* node, int id) {
        for (auto bg : node->children)
            for (auto bank : bg->children) {
                if (bank->state == State::Closed)
                    continue;
    ...
    lambda[int(Level::Rank)][int(Command::PDX)] = [] (DRAM<GDDR5_AB>* node, int id) {
        node->state = State::PowerUp;};
    lambda[int(Level::Rank)][int(Command::SRE)] = [] (DRAM<GDDR5_AB>* node, int id) {
        node->state = State::SelfRefresh;};
    lambda[int(Level::Rank)][int(Command::SRX)] = [] (DRAM<GDDR5_AB>* node, int id) {
        node->state = State::PowerUp;};
}


void GDDR5_AB::init_timing()  // @NOTE all banks of all BGs in a rank are accessed, so timings of BGs and banks at the rank level
{
    ...
    // RAS <-> RAS  // Some upgrades from Bank level to Rank level
