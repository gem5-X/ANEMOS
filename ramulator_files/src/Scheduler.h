...
public:
    ...
    void update_AB(typename T::Command cmd, const vector<int>& addr_vec, long clk)
    {
        T* spec = ctrl->channel->spec;

        auto begin = addr_vec.begin();
        auto end = begin + int(T::Level::Rank);
        vector<int> rowgroup(begin, end); // Rank, as same row is opened in all banks
        int row = *(begin + int(T::Level::Row));
        ...
        if (spec->is_closing(cmd)) {
            ...
            // The scope is always rank, as the rowtable only contains up to that scope
            int scope = int(T::Level::Rank);

            for (auto it = table.begin(); it != table.end();) {
                if (equal(begin, begin + scope, it->first.begin())) {
                    n_rm++;
                    it = table.erase(it);
                }
                else
    ...
    int get_hits_AB(const vector<int>& addr_vec, const bool to_opened_row = false)
    {
        auto begin = addr_vec.begin();
        auto end = begin + int(T::Level::Rank);
    ...
    int get_open_row_AB(const vector<int>& addr_vec) {
        auto begin = addr_vec.begin();
        auto end = begin + int(T::Level::Rank);
...
template <>
void RowTable<HBM_AB>::update(typename HBM_AB::Command cmd, const vector<int>& addr_vec, long clk);

template <>
void RowTable<HBM2_AB>::update(typename HBM2_AB::Command cmd, const vector<int>& addr_vec, long clk);

template <>
void RowTable<DDR4_AB>::update(typename DDR4_AB::Command cmd, const vector<int>& addr_vec, long clk);

template <>
void RowTable<LPDDR4_AB>::update(typename LPDDR4_AB::Command cmd, const vector<int>& addr_vec, long clk);
...
template <>
int RowTable<HBM2_AB>::get_hits(const vector<int>& addr_vec, const bool to_opened_row);

template <>
int RowTable<DDR4_AB>::get_hits(const vector<int>& addr_vec, const bool to_opened_row);

template <>
int RowTable<LPDDR4_AB>::get_hits(const vector<int>& addr_vec, const bool to_opened_row);
...
template <>
int RowTable<HBM2_AB>::get_open_row(const vector<int>& addr_vec);

template <>
int RowTable<DDR4_AB>::get_open_row(const vector<int>& addr_vec);

template <>
int RowTable<LPDDR4_AB>::get_open_row(const vector<int>& addr_vec);
